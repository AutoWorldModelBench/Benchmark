...
def load_cached_tensors(cache_path: Path) -> dict[str, torch.Tensor]:
    """Use mmap to avoid materializing the full cached split into host RAM."""
    return torch.load(cache_path, mmap=True, weights_only=False)
...
def main():
    ...
    train_tensors = load_cached_tensors(cache_dir / "train_tensors.pt")
    val_tensors = load_cached_tensors(cache_dir / "val_tensors.pt")
    eval_path = cache_dir / "val_eval_tensors.pt"
    eval_tensors = load_cached_tensors(eval_path) if eval_path.exists() else None
