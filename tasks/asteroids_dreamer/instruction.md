# Asteroids -- Dreamer World Model

## Mission

Treat the dreamer as a **starting-point world model** for Asteroids.

The model maintains a learned latent state that encodes the game history, and
predicts the **next frame** by advancing this state forward. By repeating this
process, it should serve as an **interactive forward model** of the game that
can be rolled forward in latent space and compared against the true simulator.
...
- Game: asteroids (20 max entities per frame)
- Active gameplay fields: none
- Shared cache: `/data/asteroids/_cache/`
...
`experiments/asteroids_dreamer/<run_id>/`
